...
def eviction_policy(savedir):# 图12b
...
def random_admission_pre_flash(savedir):
    ...
    for i, file in enumerate(random_admission_files):
        # data = util.load_file(file)
        dev_wr, miss_rate = get_file_pair(file)
        match = re.search(r'randomAdmissionPreSet(\d+\.?\d*)', file)
        if match:
...
def quokka(savedir):
    ...
    for i, file in enumerate(quokka_size_files):
        dev_wr, miss_rate = get_file_pair(file)
        match = re.search(r'logPer(\d+)', file)
        if match:
...
def threshold(savedir):# 图12d
    ...
    for i, file in enumerate(threshold_files):
        dev_wr, miss_rate = get_file_pair(file)
        match = re.search(r'threshold(\d+)', file)
        if match:
            label = int(match.group(1))
        else:
            label = 1
...
if __name__ == '__main__':
    ...
    if args.main_exp:# 图1b
        main_exp(savedir)
    if args.eviction_policy: # 图12b
        eviction_policy(savedir)
    if args.random_admission: # 图12a
        random_admission_pre_flash(savedir)
    if args.quokka:# 图12c
        quokka(savedir)
    if args.threshold:# 图12d
        threshold(savedir)
