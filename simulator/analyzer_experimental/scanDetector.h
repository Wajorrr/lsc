...
class ScanDetector {
   ...
   public:
    ScanDetector() = delete;
    explicit ScanDetector(parser::Parser* parser,
                          string& output_path,
                          int max_vtime_diff)
        : max_vtime_diff_(max_vtime_diff) {
        if (strcasestr(parser->trace_path_.c_str(), ".zst") != NULL) {
        ...
        is_scan_req_vec_.resize(parser->get_num_of_req(), false);
    ...
    // void add_req0(parser::Request *req) {
    //     if (unlikely(req->next_access_vtime == -2)) {
    //         std::cout << "the trace does not oracle trace, scan detector
    //         needs oracle" << std::endl; abort();
    //     }
    ...
    inline bool is_part_of_scan(struct ScanStream& ss,
                                int64_t curr_vtime,
    ...
    void add_req(parser::Request* req) {
        if (unlikely(req->next_access_vtime == -2)) {
            std::cout
                << "the trace does not oracle trace, scan detector needs oracle"
                << std::endl;
            abort();
        }
            ...
            if (n_active_stream_ >= max_vtime_diff_ - 1) {
                while (pos < n_active_stream_) {
                    if (curr_vtime > scan_stream_vec_[pos].stream_start_vtime +
                        ...
                        scan_stream_vec_[pos] =
                            scan_stream_vec_[n_active_stream_ - 1];
