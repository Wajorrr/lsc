...
namespace traceAnalyzer
{
    ...
    public:
        ...
        friend ostream &operator<<(ostream &os, const OpStat &op)
        {
            ...
            // 总请求数
            uint64_t n_req = accumulate(op.op_cnt_, op.op_cnt_ + parser::OP_INVALID + 1, 0UL);
            // 写请求次数
            uint64_t n_write =
                op.op_cnt_[parser::OP_SET] + op.op_cnt_[parser::OP_REPLACE] + op.op_cnt_[parser::OP_CAS];
            // 删除请求次数
    ...
    private:
        // 不同操作类型的出现次数
        uint64_t op_cnt_[parser::OP_INVALID + 1] = {0}; /* the number of requests of an op */
