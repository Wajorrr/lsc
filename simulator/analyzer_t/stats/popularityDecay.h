...
class PopularityDecay {
   public:
    /* params */
    int warmup_rtime_;  // 预热时间
    int time_window_;   // 时间窗口大小

    PopularityDecay(std::string& path_base,
                    int time_window = 300,
        ...
        idx_shift =
            (int)((double)warmup_rtime / time_window);  // 预热时间窗口偏移量
   ...
   private:
    ...
    std::vector<int32_t> n_req_per_window;  // 过去n个时间窗口内的请求对象数
    std::vector<int32_t> n_obj_per_window;  // 过去n个时间窗口内的新对象数

    std::ofstream stream_dump_req_ofs;
    std::ofstream stream_dump_obj_ofs;
    int idx_shift = -1;  // 除去预热时间后的起始时间窗口索引

    void stream_dump() {
#ifdef USE_REQ_METRIC
        // 输出过去n个时间窗口内，每个时间窗口的请求对象数
