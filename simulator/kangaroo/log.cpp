...
namespace flashCache
{
    ...
    Log::Log(uint64_t log_capacity, stats::LocalStatsCollector &log_stats, uint64_t readmit) : _log_stats(log_stats),
    ...
    std::vector<candidate_t> Log::insert(std::vector<candidate_t> items)
    {
        ...
        for (auto item : items)
        {
            ...
            {
                // 驱逐所有对象？这里的逻辑应该相当于每次在log中积累写入，然后满了之后一次性下刷
                // std::back_inserter(evicted)是一个插入迭代器，它在每次赋值时都会在evicted容器的末尾插入一个元素
