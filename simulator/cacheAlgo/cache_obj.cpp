 ...
 * @param cache_obj
 */
void copy_cache_obj_to_request(parser::Request *req_dest,
                               const cache_obj_t *cache_obj)
{
    req_dest->id = cache_obj->obj_id;
    req_dest->req_size = cache_obj->obj_size;
 ...
 * @param req
 */
void copy_request_to_cache_obj(cache_obj_t *cache_obj, const parser::Request *req)
{
    cache_obj->obj_size = req->req_size;
#ifdef SUPPORT_TTL
    if (req->ttl != 0)
        cache_obj->exp_time = req->time + req->ttl;
    else
        cache_obj->exp_time = 0;
#endif
    cache_obj->obj_id = req->id;
}
 ...
 * @return
 */
cache_obj_t *create_cache_obj_from_request(const parser::Request *req)
{
