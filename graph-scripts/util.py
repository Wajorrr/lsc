...
# 加载json文件，返回json文件中的对象列表
def load_file(filename):
    """ returns list of json objects, assumes file can fit in memory """
    # 使用 libconfig 格式的 .cfg 配置文件
    with open(filename) as f:
        # 使用f.read()读取文件的全部内容，并将其存储在变量data中
        data = f.read() 
        
        # 使用data.replace('}\n{', '},{')将data中的所有'}\n{'替换为'},{'
        # 这一步是为了将文件中的多个JSON对象转换为一个JSON数组
        new_data = data.replace('}\n{', '},{') 
        # print(new_data)
        
        # json.loads()是Python的json模块提供的一个函数，用于将一个JSON格式的字符串转换为Python的数据结构
        # 将经过处理的字符串转换为Python的数据结构（在这里是一个列表，列表中的每个元素都是一个字典）
