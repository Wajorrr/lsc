...
static char* req_op_str[OP_INVALID + 2] = {
    "nop",     "get",    "gets",    "set",     "add",  "cas",
    "replace", "append", "prepend", "delete",  "incr", "decr",
    "read",    "write",  "update",  "invalid", "head"};
...
static inline void free_request(parser::Request* req) {
    my_free(parser::Request, req);
}
...
class Parser {
   public:
    ...
    // 工厂模式，根据给定的配置，找到对应的负载格式，然后根据负载格式返回对应的
    // Parser 实例
    static Parser* create(const libconfig::Setting& settings);

    uint64_t get_num_of_req() { return tot_req; }
