...
void traceAnalyzer::TraceAnalyzer::initialize()
{
    ...
    // 是否统计 <重用时间经过的时间窗口数，自创建以来经过的时间窗口数> 对应的请求次数
    // 即不同访问年龄的对象被重用的概率
...
void traceAnalyzer::TraceAnalyzer::run()
{
    ...
    do
    {
        ...
        else // 当前对象出现过
        {
            ...
            // 若为写入，则是覆写请求
            if (req->type == parser::OP_SET || req->type == parser::OP_REPLACE || req->type == parser::OP_CAS)
            {
                req->overwrite = true;
...
string traceAnalyzer::TraceAnalyzer::gen_stat_str()
{
    ...
    stat_ss_ << setprecision(4) << fixed << "dat: " << parser_->trace_path << "\n"
