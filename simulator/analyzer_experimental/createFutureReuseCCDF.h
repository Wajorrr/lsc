...
#include "analyzer_utils/utils.h"
...
class CreateFutureReuseDistribution {
   public:
    ...
    inline int GET_RTIME_FROM_POS(int p) {
        if (p > fine_rtime_upbound_) {
            /* this is not a valid pos, because there is a gap between
             * fine_rtime and coarse_rtime */
            if (p <
                fine_rtime_upbound_ + fine_rtime_upbound_ / rtime_granularity_)
                return -1;
            else
   ...
   private:
    ...
    /* obj -> (write_time, read_time), read time is used to check it has been
     * read
     */
    //  unordered_map<obj_id_t, struct create_info> create_info_map_;
    robin_hood::unordered_flat_map<obj_id_t, struct create_info>
        create_info_map_;
    ...
    typedef struct {
        ...
        uint32_t
            stop_reuse_cnt;  // 该重用时间的最后一次重用次数(最后一次重用的对象是好的驱逐候选者)
        uint64_t
            reuse_access_age_sum;  // 该重用时间的正常重用的访问年龄之和(访问年龄为对象从创建到访问经过的时间)
        uint64_t
            reuse_freq_sum;  // 该重用时间的正常重用的访问频率之和(访问频率为对象的访问次数)
        uint64_t
            stop_reuse_access_age_sum;  // 该重用时间的最后一次重用的访问年龄之和
     ...
     * rtime_granularity_ */
    const int fine_rtime_upbound_ =
        3600;  // 重用时间分布下界，小于该值的重用时间不会被rtime_granularity_划分
    const int rtime_granularity_ = 5;  // 重用时间分布的划分粒度

    const int max_reuse_rtime_ = 86400 * 80;
    const int reuse_info_array_size_ =
        max_reuse_rtime_ / rtime_granularity_ +
        fine_rtime_upbound_;  // 重用时间分布的上界(最大重用时间所处的位置)
};
