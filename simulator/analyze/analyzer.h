...
#include "analyzer_stats/accessPattern.h"
#include "analyzer_stats/lifespan.h"
#include "analyzer_stats/op.h"
#include "analyzer_stats/popularity.h"
#include "analyzer_stats/popularityDecay.h"
#include "analyzer_stats/reqRate.h"
#include "analyzer_stats/reuse.h"
#include "analyzer_stats/size.h"
#include "analyzer_stats/ttl.h"

/* experimental module */
#include "analyzer_experimental/createFutureReuseCCDF.h"
#include "analyzer_experimental/lifetime.h"
#include "analyzer_experimental/probAtAge.h"
#include "analyzer_experimental/scanDetector.h"
#include "analyzer_experimental/sizeChange.h"
...
typedef struct analysis_option {
    ...
    bool
        popularity_decay;  // 是否统计过去若干个时间窗口内，每个时间窗口的请求对象数和新对象数

    bool lifetime;  // 是否统计对象生命周期

    bool
        create_future_reuse_ccdf;  // 是否统计重用时间分布(各个重用时间的访问次数、访问年龄总和、访问频率总和)

    bool prob_at_age;  // 是否统计
                       // <重用时间经过的时间窗口数，自创建以来经过的时间窗口数>
...
class TraceAnalyzer {
   public:
    explicit TraceAnalyzer(parser::Parser* parser,
                           string output_path,
        ...
        if (warmup_time_ % time_window_ != 0) {
            /* the popularityDecay computation needs warmup time to be multiple
             * of time_window */
   ...
   private:
    ...
    inline int time_to_window_idx(uint32_t rtime) {
        return rtime / time_window_;
    }
};
